...
- [HTTPヘッダ 抽象的な概念と業務イメージ](#httpヘッダ-抽象的な概念と業務イメージ)
- [レスポンスヘッダ例](#レスポンスヘッダ例)
- [Cache-Controlディレクティブの補足](#cache-controlディレクティブの補足)
...
# Cache-Controlディレクティブの補足

- **max-age**  
  レスポンスがキャッシュとして有効な秒数を指定します。例: `Cache-Control: max-age=600`（600秒間キャッシュ有効）。  
  頻繁に更新されないデータや、一定期間キャッシュしても問題ないAPIレスポンスに利用します。

- **stale-while-revalidate**  
  キャッシュが期限切れ（stale）になった後も、指定秒数だけ古いキャッシュを返しつつ、裏で新しいデータ取得（再検証）を行います。例: `Cache-Control: max-age=600, stale-while-revalidate=30`  
  ユーザー体験を損なわず、バックグラウンドで最新化したい場合に有効です。

## **使い分け例**

- `max-age`のみ: 常に新鮮なデータが必要な場合や、キャッシュ期間が明確な場合。
- `stale-while-revalidate`併用: 少し古いデータでも許容でき、レスポンス速度を優先したい場合。

```
|<------ max-age ------>|<-- stale-while-revalidate -->|
---------------------------------------------------------->
fresh (使える)          staleだが返してよい     → その後は必ず再取得
...
| 指標               | `max-age` | `stale-while-revalidate` |
| ------------------ | --------- | ------------------------ |
| 最新性重視         | △         | ○                        |
| UX（表示速度）重視 | ○         | ◎                        |
| サーバ負荷軽減     | ○         | ◎                        |
...
1. 最新性が重要→ max-age 短め（or no-cache）
1. 少し古くても体感速度優先→ max-age 長め + stale-while-revalidate
