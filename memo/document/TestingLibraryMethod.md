...
| メソッド  | 目的               | 要素が**見つからない場合** | 特徴                                                       | 使用用途の例                                                                                                                            |
| --------- | ------------------ | -------------------------- | ---------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------- |
| `getBy`   | 即座に取得         | エラーを投げる             | 同期的。要素が**確実に存在する前提**。                     | モーダル内に表示されたエラーメッセージなど、**すぐに存在しているはずの要素を検証**したいとき（例：バリデーション直後のテキスト確認）。  |
| `queryBy` | 存在確認（0 or 1） | `null` を返す              | エラーを投げず、**存在しないことを確認**したいときに便利。 | ボタンが**表示されていないことを確認**したいとき（例：権限がないユーザーには編集ボタンが出ない）。                                      |
| `findBy`  | 非同期に取得       | タイムアウト後にエラー     | `await` が必要。非同期レンダリングに対応。                 | API呼び出し後にコンポーネントが更新され、**一定時間内に要素が表示されることを確認**したいとき（例：ロード完了後にリストが描画される）。 |
